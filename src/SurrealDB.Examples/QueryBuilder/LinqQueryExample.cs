...
public class LinqQueryExample : IExample
{
	...
	public async Task RunAsync(CancellationToken cancellationToken = default)
	{
		...
		Console.WriteLine(
			SurrealQL
			   .Select<User>(user => new { user.Age, user.Name, user.Email })
			   .ExecuteAsync<User>()
		);
		...
		Console.WriteLine(
			SurrealQL
			   .Select<User>(user => new { Adult = user.Age >= 18 })
			   .ExecuteAsync<User>()
		);
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Article>(article => new { article, Tags = article.Tags.Select(tag => tag.Value) })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Customer>(customer => new { Addresses = customer.Addresses.Where(address => address.Active) })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Temperature>(temperature => new { Fahrenheit = temperature.Celsius * 2 + 30 })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<User>(user => new { MarketingSettings = new { Weekly = false, Monthly = true } })
			   .ExecuteAsync<User>()
		);
		...
		Console.WriteLine(
			SurrealQL
			   .Select<User>(userAndAdmin => new { userAndAdmin })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Article>(articleAndPost => new { articleAndPost })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Article>(article => new { Published = article.Settings.Select(settings => settings.Published) })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Person>(person => new { person.Gender, person.Country, person.City })
		...
		Console.WriteLine(
			SurrealQL
			   ...
			   .Set(
					person => new Person
					{
						Name = "Tobie", Company = "SurrealDB", Skills = new[] { "Rust", "Go", "JavaScript" }
					}
				)
			   .Return(person => new { person.Name, person.Skills })
		...
		Console.WriteLine(
			SurrealQL
			   .Select<Arr1>(arr1 => new { nums = arr1.Arr2.Select(arr2 => arr2.Arr3).First().Select(arr3 => arr3.Id) })
			   .ExecuteAsync<Arr1>()
		);
