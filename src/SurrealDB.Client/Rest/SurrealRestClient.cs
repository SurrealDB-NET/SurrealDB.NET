...
public class SurrealRestClient : ISurrealRestClient
{
	...
	public async Task<TResult> CreateRecordAsync<TResult>(
		string tableName, string id, string content, CancellationToken cancellationToken = default
	)
		where TResult : class
	{
		IEnumerable<TResult> results = await SendRequestAsync<TResult>(
			HttpMethod.Post, $"key/{tableName}/{id}", content, cancellationToken
		);
	...
	{
		IEnumerable<TResult> results = await SendRequestAsync<TResult>(
			HttpMethod.Get, $"key/{tableName}/{id}", cancellationToken
		);

		return results.FirstOrDefault();
	}

	public async Task<TResult> ModifyRecordAsync<TResult>(
		string tableName, string id, string content, CancellationToken cancellationToken = default
	)
		where TResult : class
	{
		IEnumerable<TResult> results = await SendRequestAsync<TResult>(
			HttpMethod.Patch, $"key/{tableName}/{id}", content, cancellationToken
		);

		return results.Single();
	}

	public async Task<TResult> UpsertRecordAsync<TResult>(
		string tableName, string id, string content, CancellationToken cancellationToken = default
	)
		where TResult : class
	{
		IEnumerable<TResult> results = await SendRequestAsync<TResult>(
			HttpMethod.Put, $"key/{tableName}/{id}", content, cancellationToken
		);
	...
	private async Task<IEnumerable<TResult>> ParseResponseAsync<TResult>(
	...
	private async Task SendRequestAsync(
		HttpMethod method, string endpoint, CancellationToken cancellationToken = default
	)
	{
	...
	private async Task<IEnumerable<TResult>> SendRequestAsync<TResult>(
		HttpMethod method, string endpoint, CancellationToken cancellationToken = default
	)
		where TResult : class
	{
	...
	private async Task<IEnumerable<TResult>> SendRequestAsync<TResult>(
		HttpMethod method, string endpoint, string content, CancellationToken cancellationToken = default
	)
		where TResult : class
	{
		var request = new HttpRequestMessage(method, endpoint) { Content = new StringContent(content) };
